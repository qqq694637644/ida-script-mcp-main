"""Build the U011 comment/function_comment complex guest payload."""
...
PAYLOAD_SCRIPT_NAME = "U011_comment_function_comment_complex.py"
...
def build_guest_u011_comment_function_comment_test_script(
    *,
    ...
    """Build a standalone guest script for U011 comment/function_comment testing."""
    ...
    unreplaced = [
        ...
        for token in (
            ...
            "__PLUGIN_EXPECTED_SHA256_JSON__",
        )
        if token in script
    ]
    if unreplaced:
        raise RuntimeError("Unreplaced U011 payload placeholders: " + ", ".join(unreplaced))
    return script
...
def main(argv: list[str] | None = None) -> None:
    ...
    print(f"Wrote U011 comment/function_comment guest payload: {output_path}")
