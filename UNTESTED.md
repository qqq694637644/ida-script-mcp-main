...
核心 V2.3 worker 生命周期测试 U001-U003、真实 MCP client smoke U004、多 IDA 实例选择 U005、`/functions` 主要 corner case U006、`/decompile` corner case U007、`/xrefs` corner case U008、`/inspect_address` 系统测试 U009、以及 patch_bytes 复杂情况 U013 已全部通过并移入 `TESTED.md`：
...
U006 `/functions` 主要边界语义已由 workflow run `26925694907` 覆盖并移入 `TESTED.md`；仍保留 fixture-dependent residuals。下一轮建议从 U006R、U010/U011/U012/U014 apply_changes、installer / client config coverage 开始。U008/U009 已完成，不要重复跑。
